  ...
  <div v-else-if="needsLogin" class="flex min-h-screen items-center justify-center bg-base-200 px-4">
    <section class="card w-full max-w-md bg-base-100 shadow-sm">
      <div class="card-body space-y-4 text-center">
        ...
        <AppButton variant="primary" :href="`/guxi/login?redirect=${encodeURIComponent(currentPath)}`">前往登录</AppButton>
      </div>
    </section>
  </div>
    ...
    <div class="drawer-content flex flex-col min-w-0">
      ...
      <div class="navbar bg-base-100 border-b border-base-300 w-full lg:hidden sticky top-0 z-40 shadow-sm">
        ...
        <div class="flex-1 px-2 mx-2">
          <h1 class="text-lg font-bold text-primary">{{ adminSiteName }}</h1>
        </div>
      </div>
    ...
    <div class="drawer-side z-50">
      ...
      <aside class="bg-base-100 min-h-screen w-72 flex flex-col border-r border-base-300 shadow-sm">
        ...
        >
          ...
          <div>
            <div class="text-2xl font-black text-primary tracking-tight">{{ adminSiteName }}</div>
        ...
        <div class="p-4 flex-1 overflow-y-auto">
          <ul class="menu menu-md w-full gap-1 p-0">
            <li>
              <a href="/guxi" :class="{'active': currentPath === '/guxi'}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="p-4 border-t border-base-300 mt-auto space-y-2">
          ...
          <div class="flex items-center justify-between text-xs text-base-content/50 px-2">
            <span class="cursor-default">{{ adminSiteName }}</span>
            <div class="">
              <button class="hover:text-primary transition-colors mr-2" :title="updateTip" >
...
const isLoginPage = computed(() => (pageContext.urlPathname ?? "") === "/guxi/login");
...
const adminSiteName = computed(() => pageContext.site?.siteName || "Admin");
const adminSiteSubtitle = computed(() => pageContext.site?.siteSubtitle || "Global deployment");

const breadcrumbs = computed(() => getBreadcrumbs(currentPath.value));

onMounted(() => {
  if (needsLogin.value) {
    window.location.href = `/guxi/login?redirect=${encodeURIComponent(currentPath.value)}`;
  }
});
</script>
