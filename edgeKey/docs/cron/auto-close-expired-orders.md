...
CF Store 使用 Cloudflare Workers Cron Trigger 定时扫描超时未支付的订单并自动关闭，释放库存。
...
在 Cloudflare Dashboard → Workers → cfstore → Observability 中搜索关键词：
...
1. 部署后在 Cloudflare Dashboard → Workers → cfstore → Settings → Triggers 确认 Cron 已生效
