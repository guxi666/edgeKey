...
- **网关地址**：填写易支付平台提供的api接口信息，比如下面截图叫【接口地址】就是我们要找的(注意：斜杠不要，在cf-store系统填写 https://pay.example.com即可)，系统会自动拼接 API 路径（`/submit.php`）。
