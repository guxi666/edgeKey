...
export async function updateAdminProfile(input: {
  ...
  const updateData: { username?: string; nickname?: string | null; email?: string | null; passwordHash?: string } = {};

  if (typeof input.username === "string") {
    const username = input.username.trim();
    if (!/^[A-Za-z0-9_-]{3,32}$/.test(username)) {
      throw badRequestError("用户名只能包含字母、数字、下划线和短横线，长度 3-32 位", "USERNAME_INVALID");
    }
    if (username !== admin.username) {
      const exists = await context.prisma.admin.findUnique({
        where: { username },
        select: { id: true },
      });
      if (exists && exists.id !== admin.id) {
        throw badRequestError("用户名已存在", "USERNAME_EXISTS");
      }
