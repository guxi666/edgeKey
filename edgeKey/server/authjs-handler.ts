  ...
  return {
    ...
    providers: [
      CredentialsProvider({
        name: "Credentials",
        credentials: {
          username: { label: "Username", type: "text", placeholder: "guxi" },
...
function isAdminPageContextRequest(url: string): boolean {
  ...
  return /^\/guxi\/.*\.pageContext\.json$/.test(parsed.pathname);
}
  ...
  async (request, context) => {
    if (isCredentialsCallbackRequest(request)) {
      ...
      try {
        ...
        const url = new URL(request.url);
        const callbackUrl = url.searchParams.get("callbackUrl") || "/guxi";
        const redirectUrl = new URL("/guxi/login", url.origin);
