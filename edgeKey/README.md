...
CF Store 是一套有vike框架开发，可直接部署到 Cloudflare 的一体化全栈卡密商城系统：同一套代码同时包含前端页面、SSR 渲染、后端 API / 数据变更入口，并由 Cloudflare Workers 运行。
...
> 4. 部署成功之后在页面的日志里面可以找到 "Deployed cfstore triggers (0.38 sec) https://cfstore.你的账号.workers.dev" 这样的日志，其中 "https://cfstore.你的账号.workers.dev" 就是你的项目网址。
> 5. https://cfstore.你的账号.workers.dev/guxi 为管理后台登陆地址，默认管理员账号:admin，密码:admin123456，切记登陆后立即修改密码！
...
1. [创建数据库](#如何创建数据库) 名称填 `cfstore-db`
...
```bash
sed -i 's/"database_name": "cfstore-db"/"database_name": "cfstore-db", "database_id": "你的database_id"/' wrangler.jsonc && bun run deploy
```
...
1. 进入 [dash.cloudflare.com](https://dash.cloudflare.com) → **存储和数据库** → **D1 数据库**
2. 页面右侧会展示你创建的所有数据库
3. 找到你要操作的数据库名称 比如`cfstore-db` 点击对应的`UUID`即可复制id

### 如何执行SQL

1. 进入 [dash.cloudflare.com](https://dash.cloudflare.com) → **存储和数据库** → **D1 数据库**
2. 页面右侧会展示你创建的所有数据库，点击你要操作的数据库名称 比如 `cfstore-db`
3. 点击顶部标签 → **控制台**
